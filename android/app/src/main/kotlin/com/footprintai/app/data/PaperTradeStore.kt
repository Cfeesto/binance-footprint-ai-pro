...
/**
 * PaperTradeStore — 持久化 PaperAccount 到 files 目录 JSON 文件。
 * ponytail: 手写 JSON 映射，无需 Gson/Moshi。
 */
class PaperTradeStore(ctx: Context) {
    ...
    fun save(account: PaperAccount) {
        val j = JSONObject().apply {
            ...
            put("trades",         JSONArray().also { arr -> account.trades.forEach { arr.put(it.toJson()) } })
            put("dailySnapshots", JSONArray().also { arr -> account.dailySnapshots.forEach { arr.put(it.toJson()) } })
    ...
    private fun OpenPosition.toJson() = JSONObject().apply {
        put("direction",  direction.name); put("entryPrice", entryPrice)
        put("quantity",   quantity);       put("openedAt",   openedAt)
        put("stopLoss",   stopLoss);       put("takeProfit", takeProfit)
    }
    ...
    private fun TradeRecord.toJson() = JSONObject().apply {
        put("id", id); put("direction", direction.name)
        put("entryPrice", entryPrice); put("exitPrice", exitPrice)
        put("quantity", quantity); put("pnl", pnl)
        put("openedAt", openedAt); put("closedAt", closedAt)
    ...
    private fun DailySnapshot.toJson() = JSONObject().apply { put("date", date); put("balance", balance) }
    private fun JSONObject.toSnapshot() = DailySnapshot(getString("date"), getDouble("balance"))
